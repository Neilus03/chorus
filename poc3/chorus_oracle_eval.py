...
from chorus_eval_utils import load_gt_instance_ids


SCENE_DIR = os.environ.get("SCENE_DIR", "scene0000_00")
GRANULARITIES = [float(g.strip()) for g in os.environ.get("GRANULARITIES", "0.2,0.5,0.8").split(",")]
ORACLE_MIN_IOU_FOR_PLY = float(os.environ.get("ORACLE_MIN_IOU_FOR_PLY", "0.1"))


def load_all_proposals(scene_dir: str, granularities, return_sources: bool = False):
    all_proposals = []
    proposal_sources = []
    current_max_id = 0
...
def evaluate_oracle_ap(gt_ids, proposals, thresholds=(0.25, 0.50)):
    ...
    results = {}
    print(f"Evaluating CHORUS pool of {len(proposals)} proposals against {len(gt_instances)} GT objects...")
...
def main():
    ...
    print(f"--- CHORUS ORACLE EVALUATION: {scene_name} ---")

    gt_ids = load_gt_instance_ids(scene_dir, scene_name)
    ...
    for bucket, metrics in oracle_results.items():
        print(
            f"{bucket:<25} | {metrics['Count']:<8} | "
            f"{metrics['AP25']:<12.4f} | {metrics['AP50']:<12.4f}"
        )
    print("=" * 65)
    ...
    print("Interpretation: maximum potential of the CHORUS proposal pool.\n")

    results_to_save = dict(oracle_results)
    results_to_save["_extras"] = additional_metrics
    with (scene_dir / "oracle_metrics.json").open("w", encoding="utf-8") as f:
