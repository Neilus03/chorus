...
SVD_COMPONENTS = int(os.environ.get("SVD_COMPONENTS", "32"))
...
def main() -> None:
    ...
    print("CHORUS: projecting 2D masks to 3D...")
    row_idx = []
    col_idx = []
    ...
    if global_mask_counter == 0:
        raise RuntimeError("No masks were bridged to 3D. Check CHORUS teacher outputs.")
    ...
    print(
        f"DEBUG: {unseen_points} points ({unseen_points/num_points*100:.1f}%) were NEVER seen by camera."
    )

    n_components = min(SVD_COMPONENTS, max(2, global_mask_counter - 1))
    print(f"CHORUS: SVD compression (n_components={n_components})...")
    svd = TruncatedSVD(n_components=n_components, random_state=42)
    point_features = svd.fit_transform(point_mask_matrix)

    print("CHORUS: normalizing feature vectors...")
    ...
    print("CHORUS: HDBSCAN clustering...")
    clusterer = HDBSCAN(
