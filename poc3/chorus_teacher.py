...
DEVICE = os.environ.get("CHORUS_DEVICE", "cuda:0")
GRANULARITY = float(os.environ.get("GRANULARITY", "0.8"))
DEBUG_FIRST_N_FRAMES = int(os.environ.get("DEBUG_FIRST_N_FRAMES", "10"))

UNSAM_ROOT = Path(
    os.environ.get("UNSAM_ROOT", str(Path(__file__).resolve().parent / "UnSAMv2"))
)
SAM2_PYTHON_ROOT = UNSAM_ROOT / "sam2"
DEFAULT_SCRATCH_CKPT = Path(
    f"/scratch2/{os.environ.get('USER', 'nedela')}/chorus_poc/checkpoints/unsamv2/unsamv2_plus_ckpt.pt"
)
LEGACY_SCRATCH_CKPT = Path(
...
CHECKPOINT_PATH = Path(
    os.environ.get(
        ...
        str(
            ...
            else (LEGACY_SCRATCH_CKPT if LEGACY_SCRATCH_CKPT.exists() else DEFAULT_LOCAL_CKPT)
        ),
    )
)
...
if not SAM2_PYTHON_ROOT.exists():
    raise FileNotFoundError(
        ...
        "Set UNSAM_ROOT or place UnSAMv2 under poc3."
    )
...
def main() -> None:
    if not CHECKPOINT_PATH.exists():
        raise FileNotFoundError(
            ...
            "Download checkpoint from UnSAMv2 model zoo and set UNSAMV2_CKPT."
        )

    print(f"Loading CHORUS teacher (UnSAMv2) on {DEVICE}...")
    ...
    print(f"CHORUS teacher: processing {len(target_frames)} frames at g={GRANULARITY}...")
    total_saved_masks = 0
    ...
    print(f"CHORUS teacher complete. Total saved masks: {total_saved_masks}")
