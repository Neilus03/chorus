...
from chorus.orchestrators.streaming import read_scannetpp_scene_ids, run_streaming_scannetpp
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional txt file with one ScanNet++ scene id per line.",
    )
    parser.add_argument(
        "--split",
        type=str,
        default=os.environ.get("CHORUS_SCANNETPP_SPLIT", "nvs_sem_val"),
        help="Official ScanNet++ split to read when --scene-list-file is not provided.",
    )
    parser.add_argument(
        "--max-scenes",
        type=int,
    ...
    parser.add_argument(
        ...
        help="ScanNet++ oracle benchmark filter.",
    )
    parser.add_argument(
        "--device",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Directory where W&B run files/logs are written.",
    )
...
def main() -> None:
    ...
    evaluation_hooks = ScanNetPPEvaluationHooks(args.scannetpp_eval_benchmark)
    ...
    if report_dir is None:
        report_dir = dataset_root / "_chorus_reports"
    ...
    print(f"scannetpp_eval_benchmark={evaluation_hooks.eval_benchmark}")
    ...
    wandb_reporter = WandbReporter(
        ...
        run_name=f"chorus_streaming_scannetpp_{timestamp}",
        wandb_dir=wandb_dir,
        config={
    ...
    try:
        run_summary = run_streaming_scannetpp(
            dataset_root=dataset_root,
            scene_ids=scene_ids,
            teacher=teacher,
            granularities=granularities,
